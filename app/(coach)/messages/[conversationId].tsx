...
export default function CoachMessageThreadScreen() {
  ...
  const [athleteLastActive, setAthleteLastActive] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setAthleteName(athlete?.full_name ?? 'Athlete');
        setAthleteLastActive(athlete?.last_active_at ?? null);
        ...
        await supabase.from('coach_accounts').update({ last_active_at: new Date().toISOString() }).eq('id', coachId);
      } catch (e) {
  ...
  const appendAndSend = async (opts: {
    ...
    if (!coach?.id || !athleteId || !conversationId) return false;
    setSending(true);
    setSendError('');
    try {
      const { data: msg, error } = await supabase.rpc('send_coach_message', {
  ...
  const handleSend = async () => {
    ...
    // it first meant a failed send silently lost whatever was typed.
